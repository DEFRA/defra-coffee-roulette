  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    document.getElementById("pair-btn").onclick = function () {
      ...
      if (
        isNaN(groupSize) ||
        groupSize < 2 ||
        groupSize > currentEmails.length
      ) {
        showAlert(
          "Please enter a valid group size between 2 and " +
      ...
      if (remainder !== 0) {
        ...
        showAlert(
          `The number of participants (${currentEmails.length}) is not divisible by the group size (${groupSize}). ` +
